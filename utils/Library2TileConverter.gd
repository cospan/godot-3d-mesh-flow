...
## This script is designed to handle the conversion of library data to tile
## data, manage progress updates, and interact wit ha database. It uses signals
## to communicate progress and completion, and it maintains several dictionaris
## to map face indicies to socket IDs, including handling expanded and
## reflected data.
##############################################################################
# Signals
##############################################################################
...
var m_db_library_adapter
var m_db_tile_adapter
...
func process_database(db_adapter, db_wfc_adapter):
    m_db_library_adapter = db_adapter
    m_db_tile_adapter = db_wfc_adapter
    ...
    match(m_state):
        STATE_T.IDLE:
            if m_flag_process_database:
                ...
                #m_existing_module_face_sid_dict = m_db_tile_adapter.get_module_dict()
                #m_existing_module_hash_dict = m_db_tile_adapter.get_module_hash_dict()
...
func _start_generate_module_dict_from_database():
    ...
    var sids_hash_dict = m_db_library_adapter.get_sids_hash_dict()
    ...
    #m_db_tile_adapter.insert_modules(m_module_face_sid_dict)
    percent = 100
    call_deferred("emit_percent_update", _pname, percent)
...
func _start_insert_database():
    ...
    m_db_tile_adapter.clear_tables()
    var d3d_size = m_db_library_adapter.get_default_size_3d()
    m_db_tile_adapter.set_default_size_3d(d3d_size)
    var mesh_dir_path = m_db_library_adapter.get_mesh_directory()
    m_db_tile_adapter.set_mesh_dir_path(mesh_dir_path)
    ...
    for sid in m_reflected_sid_dict.keys():
        m_db_tile_adapter.insert_reflected_sid(sid, m_reflected_sid_dict[sid])
        index+= 1
        percent = (float((index)) / total_size) * 100.0
        call_deferred("emit_percent_update", _pname, percent)
        await continue_step
    ...
    for sid in sid_dict.keys():
        m_db_tile_adapter.insert_sid_mapping(sid, sid_dict[sid]["asymmetric"], sid_dict[sid]["module_list"])
        index += 1
        percent = (float((index)) / total_size) * 100.0
        call_deferred("emit_percent_update", _pname, percent)
        await continue_step

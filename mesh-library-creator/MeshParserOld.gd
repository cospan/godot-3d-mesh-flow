...
@export_dir var MESH_DATA_DIR:String = "res://assets/test_pack/"



var m_force_new_db:bool = false


# ba = Base Agnostic
var DEBUG = false
#var DEBUG = true
var STATE_DEBUG = true
#var STATE_DEBUG = false
#var DEBUG_MODULE = "tile021"
var DEBUG_MODULE = "nothing"
var DEBUG_ENABLE_MODULE = false
var BACKGROUND_THREAD_ENABLE = false
#var BACKGROUND_THREAD_ENABLE = true

var m_preview_generator

var m_new_sids_module_face_dict = {}
var m_new_module_face_sids_dict = {}

var m_old_sids_module_face_dict = {}
var m_old_module_face_sids_dict = {}
...
enum STATE_T {
    ...
    HASH_LIBRARY,
    ANALYZE_HASH_LIBRARY,
    GENERATE_BASE_AGNOSTIC_HASHES,
    ANALYZE_BASE_AGNOSTIC_HASHES,
    GENERATE_INDEX_MAP,
    UPDATE_DATABASE,
    EXPORT
}
...
#If set this will clear the database table
#var m_flag_reset_module_table = false
...
# Issues List
var m_possible_face_issue_buffer = []

# Background Thread
var m_background_thread_handle = null

# Database
var m_db_adapter = null

var m_exported_mesh_library = null
...
# Public Functions
##############################################################################
func start_loading_library(mesh_dir:String, force_new_db:bool = false):
    if m_state == STATE_T.IDLE:
        m_flag_start_loading_library = true
        MESH_DATA_DIR = mesh_dir
        m_force_new_db = force_new_db

func export_mesh_library():
    if m_state == STATE_T.IDLE:
        m_flag_begin_export = true

func enable_expanded_export_library(enable:bool):
		m_flag_export_library_expand = enable

func backup_database(path:String, _name:String = m_db_adapter.DATABASE_NAME):
    m_db_adapter.backup_to(path + _name)

func get_default_size() -> Vector2:
    return m_db_adapter.get_default_size_2d()

func get_hash_from_module_name_and_face(_module_name:String, _face:FACE_T, _base_agnostic:bool = false) -> String:
    return m_db_adapter.get_hash_from_module_name_and_face(_module_name, _face, _base_agnostic)
...
func get_hashes_from_sid(_index:int, _base_agnostic:bool = false):
...
func is_module_face_symmetrical(_module_name:String, _face:FACE_T):
    var hash_dict = m_db_adapter.get_hash_dict_from_module_name_and_face(_module_name, _face)
    if hash_dict == null:
        return false
...
func _ready():
    m_preview_generator = $PreviewGenerator
    m_db_adapter = $ModuleDatabaseAdapter
    m_hash_ctx = HashingContext.new()
    m_logger = LogStream.new("Module Parser")
    m_logger.current_log_level = Log.LogLevel.INFO
    if DEBUG:
      m_logger.current_log_level = Log.LogLevel.DEBUG
...
func _process(_delta):
    ...
    match m_state:
        STATE_T.RESET:
            pass
        STATE_T.IDLE:
            if m_flag_start_loading_library and is_node_ready():
                m_logger.info("Starting to load the library")
                m_flag_start_loading_library = false
                # Reset all the variables
                m_flag_async_finished = false

                var fs_info_dict = get_mesh_file_info_from_dir(MESH_DATA_DIR)
                m_mesh_dict = get_mesh_dict_from_dir(MESH_DATA_DIR)
                m_db_adapter.open_database(MESH_DATA_DIR, m_force_new_db)
                var db_info_dict = m_db_adapter.get_file_info_dict()
                var novel_modules = find_novel_mesh_files(fs_info_dict, db_info_dict)
                m_novel_modules = novel_modules.keys()
                m_db_adapter.set_file_info_dict(novel_modules)

                if BACKGROUND_THREAD_ENABLE:
                    m_background_thread_handle = TaskManager.create_task(_find_novel_modules_bounds, true, "Background Thread")
                else:
                    _find_novel_modules_bounds()
                m_logger.debug("_process: IDLE -> FIND_MODULE_BOUNDS")
                m_state = STATE_T.FIND_MODULE_BOUNDS

            elif m_flag_begin_export:
                #XXX: Start the export operation
                m_flag_begin_export = false
                m_state = STATE_T.EXPORT
                _export_mesh_library()
        ...
        STATE_T.FIND_MODULE_BOUNDS:
            if not m_flag_async_finished:
                ...
                if BACKGROUND_THREAD_ENABLE:
                    m_background_thread_handle = TaskManager.create_task(_find_library_module_bounds, true, "Background Thread")
                else:
                    _find_library_module_bounds()
                m_logger.debug("_process: FIND_MODULE_BOUNDS -> FIND_DEFAULT_SIZE")
        ...
        STATE_T.FIND_DEFAULT_SIZE:
            if not m_flag_async_finished:
                ...
                if BACKGROUND_THREAD_ENABLE:
                    m_background_thread_handle = TaskManager.create_task(_create_2d_face_library, true, "Background Thread")
                else:
                    _create_2d_face_library()
                m_logger.debug("_process: FIND_DEFAULT_SIZE -> PARSE_FACE")
        STATE_T.PARSE_FACE:
            if not m_flag_async_finished:
                emit_signal("continue_step")
            else:
                ...
                if BACKGROUND_THREAD_ENABLE:
                    m_background_thread_handle = TaskManager.create_task(_start_base_offset_removal, true, "Background Thread")
                else:
                    _start_base_offset_removal()
                m_logger.debug("_process: PARSE_FACE -> GENERATE_VERTEX_LISTS")
        STATE_T.GENERATE_VERTEX_LISTS:
            if not m_flag_async_finished:
                ...
                if BACKGROUND_THREAD_ENABLE:
                    m_background_thread_handle = TaskManager.create_task(_start_library_hash, true, "Background Thread")
                else:
                    _start_library_hash()
                m_logger.debug("_process: GENERATE_VERTEX_LISTS -> HASH_LIBRARY")
        STATE_T.HASH_LIBRARY:
            if not m_flag_async_finished:
                ...
                if BACKGROUND_THREAD_ENABLE:
                    m_background_thread_handle = TaskManager.create_task(_start_base_agnostic_library_hash, true, "Background Thread")
                else:
                    _start_base_agnostic_library_hash()
                m_logger.debug("_process: HASH_LIBRARY -> GENERATE_BASE_AGNOSTIC_HASHES")
        STATE_T.GENERATE_BASE_AGNOSTIC_HASHES:
            if not m_flag_async_finished:
                ...
                if BACKGROUND_THREAD_ENABLE:
                    m_background_thread_handle = TaskManager.create_task(_start_generating_index_dict, true, "Background Thread")
                else:
                    _start_generating_index_dict()
                m_logger.debug("_process: GENERATE_BASE_AGNOSTIC_HASHES -> GENERATE_INDEX_MAP")
        STATE_T.GENERATE_INDEX_MAP:
            if not m_flag_async_finished:
                ...
                m_state = STATE_T.IDLE
                m_logger.debug("_process: GENERATE_INDEX_MAP -> IDLE")
                m_logger.info("Finished Initializing")
        STATE_T.UPDATE_DATABASE:
            if not m_flag_async_finished:
                emit_signal("continue_step")
            else:
                ...
                m_state = STATE_T.IDLE
                if STATE_DEBUG:
                    m_logger.debug ("m_mesh_parser::_process: UPDATE_DATABASE -> IDLE")
                m_logger.info ("Finished Updating Database")
        STATE_T.EXPORT:
            if not m_flag_async_finished:
                emit_signal("continue_step")
            else:
                m_state = STATE_T.IDLE
                if STATE_DEBUG:
                    m_logger.debug ("m_mesh_parser::_process: EXPORT -> IDLE")
                m_logger.info ("Finished Export")
        _: # Default
            pass
...
# - Goes through each mesh and finds the faces that are on the outside of the mesh
# - Finds the module bounds by going through all modules and finding the most common start and end points
#         - XXX: It might be better to use the maximum size of the modules instead of the most common start and end points
##############################################################################
func get_mesh_file_info_from_dir(_mesh_data_dir:String):
    var mesh_info_dict = {}
...
func find_novel_mesh_files(fs_dict: Dictionary, db_dict: Dictionary) -> Dictionary:
    var output_dict = {}
...
func _find_library_module_bounds():
    var sp_x = {}
    var sp_z = {}
    var sp_y = {}
    var ep_x = {}
...
# - m_verticies_dict
# - m_r_verticies_dict
# - m_flag_async_finished
#
# - Create a list of vertices for each face as well as the reflection of the vertices
##############################################################################h
...
# Module 2D Face Hash
#
# - Goes through each module and finds the hash of each face
##############################################################################
...
#
# - Goes through each face and finds the faces that are the same
##############################################################################
...
## Export Functions
###############################################################################
func _export_mesh_library():
    var percent = 0.0
    var total_size = len(m_mesh_dict.keys())
		if m_flag_export_library_expand:
				print ("XXX: IMPLEMENT EXPANDED DATABASE!!!")
    m_exported_mesh_library = MeshLibrary.new()
    #var tmesh_name = m_mesh_dict.keys()[0]
    #var tmesh = m_mesh_dict[tmesh_name]
    var _pname = PROGRESS_UPDATE_EXPORT_LIBRARY
    call_deferred("emit_percent_update", _pname, percent)
    m_logger.info("Exporting Mesh...")
    for i in range (len(m_mesh_dict.keys())):
        percent = (float((i + 1)) / total_size) * 100.0
        var mesh_name = m_mesh_dict.keys()[i]
        var mesh_instance = m_mesh_dict[mesh_name]
        var mi = MeshInstance3D.new()
        mi.set_mesh(mesh_instance.mesh.duplicate())
        m_exported_mesh_library.create_item(i)
        m_exported_mesh_library.set_item_mesh(i, mi.mesh)
        m_exported_mesh_library.set_item_name(i, mesh_name)
        m_preview_generator.start_preview_capture(mi, Vector2i(512, 512))
        await m_preview_generator.texture_finished
        m_exported_mesh_library.set_item_preview(i, m_preview_generator.get_preview_texture())
        call_deferred("emit_percent_update", _pname, percent)  
        #if not BACKGROUND_THREAD_ENABLE:
        #    await continue_step  
